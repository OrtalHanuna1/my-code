...
model = faster_vit_0_any_res(pretrained=False).to(device)
...
for i, image_path in enumerate(all_image_paths):
    ...
    segmented_image = cv2.resize(segmented_image, (image_copy.shape[1], image_copy.shape[0]))
    
    final_image = image_overlay(image_copy, segmented_image)
